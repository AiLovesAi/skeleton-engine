...
namespace game {
    ...
    std::shared_ptr<uint8_t> BKV::generateCompound(const UTF8Str& name) {
    ...
    template<typename T>
    std::shared_ptr<uint8_t> BKV::generate(const UTF8Str& name, const T data) {
    ...
    template<typename T>
    std::shared_ptr<uint8_t> BKV::generateList(const UTF8Str& name, const T* data, uint32_t size) {
        static_assert(std::is_arithmetic<T>::value || std::is_enum<T>::value,
    ...
    std::shared_ptr<uint8_t> BKV::generateStr(const UTF8Str& name, const UTF8Str& data) {
    ...
    std::shared_ptr<uint8_t> BKV::generateStrList(const UTF8Str& name, const UTF8Str* data, const uint32_t size) {
        size_t c = 2 + name.len + sizeof(uint32_t);
        size_t head = 0;
